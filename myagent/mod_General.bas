...
Lcolor = RGB(0, 0, 0) 'black is transparent color of frames
    hRgn = GetBitmapRegion(fRenderForm.Picture, Lcolor)
...
Public Function DoAnimation(sName As String) As Boolean
...
sActionData = GetLinEle(OpenTextFile(App.Path & "\temp\info.dat"), "<" & LCase(sName) & ">", "</" & LCase(sName) & ">")
...
If sFrame(i) <> "" Then
    ...
    Else
        ...
        If CheckFile(App.Path & "\temp\" & sName & "\" & sFrameData(1)) = True Then
            ...
            frmOffScreen.Picture = LoadPicture(App.Path & "\temp\" & sName & "\" & sFrameData(1))
